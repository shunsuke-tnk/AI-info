...
├── public/images/articles/        # 記事サムネイル画像（PNG）
├── scripts/
│   ├── generate-thumbnail.sh      # サムネイル生成スクリプト
│   └── thumbnail-template.html    # サムネイルHTMLテンプレート
├── src/
...
**読者像**: 初心者。記事を読みながら内容を理解し、実践できることがゴール。

**基本方針**:
- 文字数制限なし。内容が伝わるか、実践のマニュアルになるかが基準
- 読みやすさ > 簡潔さ。必要なら長く書いてよい

**専門用語の扱い**:
- 初出時に括弧書きで注釈を入れる（例: `コンテキストウィンドウ（一度に処理できる情報量の上限）`）
- 英語の専門用語も日本語で補足する（例: `.gitignore（Gitで管理しないファイルのリスト）`）

**Calloutコンポーネントの使用**:
- 難しい概念の後に `<Callout type="info">` で砕いた解説を入れる
- 実践のコツは `<Callout type="tip">` で強調する
- セキュリティや破壊的操作の注意は `<Callout type="warning">` で警告する

**コードブロック**:
- ターミナルで実行するコマンドは必ず言語指定付きコードブロックで表示する（コピーボタンが自動付与される）
- `bash` または `powershell` の言語指定を必ず付ける
- コマンドの前後に何をしているかの説明を入れる

**記事の構成**:
- 「やってみよう」形式で、読みながら実践できる手順を含める
- 基礎 → 応用の順に段階的に難易度を上げる
- 上級者向けの機能は「まずは不要」と明示し、初心者が圧倒されないようにする
- 各セクション冒頭で「このセクションで何ができるようになるか」を示す

**MDXコンポーネント**:
- `<Callout type="info|tip|warning" title="タイトル">内容</Callout>` が使用可能
- コードブロックには自動でコピーボタンが付く（CodeBlockコンポーネント）

**サムネイル画像**:
- 全記事で統一デザインのサムネイルを使用する（インディゴ〜バイオレット〜マゼンタのグラデーション背景 + 白文字タイトル）
- 生成コマンド: `./scripts/generate-thumbnail.sh "表示タイトル" "CATEGORY" "ファイル名"`
- カテゴリラベルは英語大文字で指定: `AUTOMATION` / `USE CASES` / `AI DEV` / `SETUP` / `FRONTIER`
- タイトルが長い場合は `<br>` で改行位置を手動指定する（自然な日本語の区切りで）
- 出力形式: PNG、1200x630px
- フロントマターの `thumbnail` は `/images/articles/ファイル名.png` で指定
- 記事を新規作成するたびにサムネイルも生成すること
...
- [x] メタデータ・JSON-LD・OGP
- [x] サイトマップ + robots.txt
- [x] Vercelデプロイ (https://ai-shelf.vercel.app)
- [ ] Lighthouse 90点以上
